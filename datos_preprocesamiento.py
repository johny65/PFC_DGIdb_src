...
def cargar_aliases(aliases_ruta):
    '''
...
def aliases_repeticiones(aliases_conjunto,aliases_lista,salida):
    '''
    Guarda en un archivo los aliases de genes/drogas repetidos
    '''
...
def ocurrencias(entidades,publicaciones,embeddings,salida):
    '''
...
def etiquetas_publicacion(pmids_lista,ifg_lista,aliases_lista,salida):
    with open(salida,"w",encoding="utf8") as salida_csv:
        escritor_csv = csv.writer(salida_csv,delimiter=',',lineterminator="\n")
        lista = list()
        for pmid in pmids_lista:
...
if __name__ == "__main__":
    ...
    aliases_gen_conjunto = cargar_aliases(aliases_gen_ruta)
    ...
    # ocurrencias(aliases_gen,publicaciones_dict,embeddings_dict,"ocurrencias_genes.csv")
    ...
    # ifg_csv_ruta = "E:/Descargas/Python/PFC_DGIdb_src/ifg.csv"
    # ifg_lista = cargar_ifg(ifg_csv_ruta)
    # print(ifg_lista)
    # print("Interacciones fármaco-gen cargadas")

    # etiquetas_publicacion(pmids_lista,ifg_lista,aliases_gen_lista,"etiquetas_publicaciones.csv")